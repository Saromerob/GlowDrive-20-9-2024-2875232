...
<section class="registro">
    ...
    <form action="../../repository/registro_usuario.php" method="post">
        Nombres: <input class="entrada-control" type="text" name="nombre" pattern="[A-Za-z\s]+" placeholder="Ingrese nombres" required autocomplete="off">
        Apellidos: <input class="entrada-control" type="text" name="apellido" pattern="[A-Za-z\s]+" placeholder="Ingrese Apellido" required autocomplete="off">
        <label for="tipo_documento_id">Tipo de Documento:</label>
        <select class="entrada-control" name="tipo_documento_id" id="tipo_documento_id" required autocomplete="off"><br><br>
            <?php foreach ($typeDocuments as $typeDocument): ?>
            <option value="<?php echo $typeDocument['id']; ?>"><?php echo $typeDocument['tipo']; ?></option>
            <?php endforeach; ?>
        </select><br>
        Número de Documento: <input class="entrada-control" type="number" name="num_documento" id="numdoc" pattern="[0-9]+" minlength="7" maxlength="18" placeholder="Número de Documento" required autocomplete="off">
        Teléfono: <input class="entrada-control" type="number" name="telefono" id="Telefono" pattern="[0-9]+" minlength="7" maxlength="10" placeholder="Ingrese Teléfono" required autocomplete="off">
        Correo Electrónico: <input class="entrada-control" type="email" name="correo" id="correo" placeholder="Ingrese Correo" required autocomplete="off">
        Contraseña: <input class="entrada-control" type="password" name="contrasena" id="contrasena" minlength="8" maxlength="8" placeholder="Ingrese Contraseña" required autocomplete="off">
        Fecha de Nacimiento: <input class="entrada-control" type="date" name="fecha_nacimiento" required autocomplete="off">
        <label for="localidad_id">Localidad:</label>
        <select class="entrada-control" name="localidad_id" id="localidad_id" autocomplete="off">
            <?php foreach ($localidades as $localidad): ?>
            <option value="<?php echo $localidad['id']; ?>"><?php echo $localidad['nombre']; ?></option>
            <?php endforeach; ?>
        </select><br><br>
        <input class="entrada-control-checkbox" type="checkbox" required> Acepto los <a href="#">Términos y Condiciones</a>
        <input class="boton" type="submit" name="registro" id="registro" value="Registrarse">
        <p>Si ya tienes una cuenta <a href="sesion.php">Inicia sesión</a></p>
    </form><br>
</section>
